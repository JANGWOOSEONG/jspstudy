...
public class BoardDAO {
	...
	// Singleton Pattern으로 DAO 생성하기 (객체를 하나만 만들어놓고 쓰겠다.)
	private static BoardDAO dao = new BoardDAO(); // 생성자 프라이빗 처리
	private BoardDAO() {
	//context.xml에서 <Resource name = "jdbc/GDJ61" /> Resource를 읽어서 Data	Source 객체 생성하기(JNDI 방식)
	try {
		Context context = new InitialContext(); 
		Context envContext = (Context)context.lookup("java:comp/env");
		dataSource = (DataSource)envContext.lookup("jdbc/GDJ61");
		/*
			Context context = new InititalContext();
	...
	public static BoardDAO getInstance() { //스태틱 메소드는 스태틱만 사용가능
		return dao;
	}
	...
	public List<BoardDTO> selectBoardList(){
		...
		try {
			...
			// 6. ResultSet 객체(결과 집합)를 ArrayList를 만듬
			while(rs.next()) {
	...
	public int insertBoard(BoardDTO board) {
		...
		try {
			...
			// 5. 쿼리문에 변수 값 전달하기
			ps.setString(1, board.getTitle()); // 앞에 1은 1번째 물음표(?)에 title 전달하기
			ps.setString(2, board.getContent()); // 앞에 2는 2번째 물음표(?)에 content 전달하기
